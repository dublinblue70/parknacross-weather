(() => {
 ...
 function coverageText(rows,requestedHours){const epochs=rows.map(rowEpoch).filter(Number.isFinite).sort((a,b)=>a-b);if(!epochs.length)return"No usable timestamps";const available=Math.max(0,epochs.at(-1)-epochs[0]),requested=requestedHours*3600,pct=Math.min(100,available/requested*100);const date=e=>new Date(e*1000).toLocaleString("en-IE",{timeZone:"Europe/Dublin",day:"numeric",month:"short",hour:"2-digit",minute:"2-digit"});return `${date(epochs[0])}–${date(epochs.at(-1))} · ${pct.toFixed(1)}% of selected span`;}
 ...
 async function load(h){hours=h;set("graphRangeTitle",({6:"Last 6 hours",24:"Last 24 hours",48:"Last 48 hours",168:"Last 7 days",720:"Last 30 days"})[h]);set("graphCoverage","Checking available coverage…");set("graphCount","Loading…");
 ...
 const coverage=coverageText(rows,h);set("graphCoverage",coverage);set("graphCount",`${rows.length.toLocaleString("en-IE")} saved observations · ${r.filter(x=>!x?._gap).length.toLocaleString("en-IE")} extrema-preserving points plotted${gapText}${qualityText}${gustQualityText}`);set("chartTextSummary",`${({6:"Six-hour",24:"Twenty-four-hour",48:"Forty-eight-hour",168:"Seven-day",720:"Thirty-day"})[h]} charts. Available observations cover ${coverage}. ${gapData.gaps?`${gapData.gaps} archive gap${gapData.gaps===1?" is":"s are"} shown as breaks.`:"No archive gaps longer than twenty minutes."}`);}catch(e){set("graphCount","Archive temporarily unavailable.");set("graphCoverage","Coverage unavailable.");}}
