...
function updateDashboard(current) {
  ...
  set("battery", usable(current.battery_v) ? `${n(current.battery_v, 2)} V` : "--");
...
async function loadEverything() {
  try {
    /*
     * Current conditions and 24h history are the essential feeds.
     * The longer-range history and new stats endpoint are optional so
     * the main dashboard still works while the upgraded Worker is being deployed.
     */
    const [current, history24Response] = await Promise.all([
      getJSON(CURRENT_URL),
      getJSON(HISTORY_24_URL)
    ]);

    history24 = Array.isArray(history24Response.readings)
      ? history24Response.readings
      : [];

    try {
      const history7Response = await getJSON(HISTORY_7D_URL);
      history7d = Array.isArray(history7Response.readings)
        ? history7Response.readings
        : [];
    } catch (error) {
      console.warn("7-day history unavailable:", error);
      history7d = [];
    }
    ...
    try {
      ...
      console.warn("Extended station stats unavailable:", error);
      stats = null;
    }
    ...
    console.error("Parknacross Weather:", error);
