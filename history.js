...
function renderHeatmap(){
  ...
  for(let i=0;i<371;i++){
    ...
    const row=dailyMap.get(day);const button=document.createElement("button");button.type="button";button.className="heatmap-day";button.setAttribute("aria-label",day);
    if(row&&usable(row.sample_count)){
      const expected=expectedSamples(day),actual=Number(row.sample_count),pct=Math.min(100,(actual/expected)*100);populated++;totalExpected+=expected;totalActual+=actual;if(pct>=98)complete++;
      const level=pct>=98?4:pct>=90?3:pct>=50?2:1;button.classList.add("has-data",`level-${level}`);button.title=`${longDay(day)} · ${actual}/${expected} saves · ${pct.toFixed(1)}% complete`;button.addEventListener("click",()=>openArchiveDay(day));
    }else button.title=`${longDay(day)} · no archived data`;
    if(day===todayKey())button.classList.add("current");if(day===selectedArchiveDay)button.classList.add("selected");host.appendChild(button);
  }
  const pct=totalExpected?Math.min(100,totalActual/totalExpected*100):null;
  set("heatmapSummary",populated?`${populated} archived day${populated===1?"":"s"} in view · ${complete} at ≥98% completeness${pct!==null?` · ${pct.toFixed(1)}% of scheduled saves captured across populated days`:""}.` : "The completeness map will fill as the archive grows.");
}

function renderCalendar(){const host=$("archiveCalendar");if(!host||!calendarCursor)return;host.innerHTML="";["Mon","Tue","Wed","Thu","Fri","Sat","Sun"].forEach(name=>{const h=document.createElement("div");h.className="cal-head";h.textContent=name;host.appendChild(h);});const first=new Date(Date.UTC(calendarCursor.year,calendarCursor.month,1,12)),daysInMonth=new Date(Date.UTC(calendarCursor.year,calendarCursor.month+1,0,12)).getUTCDate(),offset=(first.getUTCDay()+6)%7;set("calendarTitle",first.toLocaleDateString("en-IE",{timeZone:"UTC",month:"long",year:"numeric"}));for(let i=0;i<offset;i++){const blank=document.createElement("button");blank.type="button";blank.className="archive-day empty";blank.disabled=true;host.appendChild(blank);}for(let day=1;day<=daysInMonth;day++){const key=`${monthKey(calendarCursor)}-${String(day).padStart(2,"0")}`,row=dailyMap.get(key),button=document.createElement("button");button.type="button";button.className=`archive-day ${row?"has-data":"no-data"}${key===selectedArchiveDay?" active":""}`;button.disabled=!row;const rain=usable(row?.rain_mm)?`${n(row.rain_mm)} mm`:"",range=usable(row?.high_c)&&usable(row?.low_c)?`${n(row.low_c)}°–${n(row.high_c)}°`:"";button.innerHTML=`<span class="day-num">${day}</span><small>${[range,rain].filter(Boolean).join("<br>")||"No data"}</small>`;if(row)button.addEventListener("click",()=>openArchiveDay(key));host.appendChild(button);}}
...
function renderDayChart(rows){
  ...
  if(!plottable.length){
    clearDayChart(validRows.length?"Detailed observations exist for this date, but no temperature or rain-rate points are available to plot.":"No detailed observation samples are available for this date.");
    return 0;
  }
  const chart=charts.day;
...
async function loadDay(day){
  ...
  clearDayChart("");
  try{
    const data=await getJSON(`${API_BASE}/day?date=${encodeURIComponent(day)}`),s=data.summary||{},rows=Array.isArray(data.readings)?data.readings:[];
    if(requestId !== dayRequestId || selectedArchiveDay !== day)return;
    ...
    if(data.available){
      set("archiveSearchStatus",plotted?`Showing ${longDay(day)} · ${plotted.toLocaleString("en-IE")} plotted observations.`:`Showing ${longDay(day)} · no plottable detailed observations.`);
    }else{
      set("archiveSearchStatus",`No stored observations are available for ${longDay(day)}.`);
      clearDayChart("No detailed observation samples are available for this date.");
    }
  }catch(error){
    if(requestId !== dayRequestId || selectedArchiveDay !== day)return;
...
async function loadHistory(){try{const[daily,stats,events]=await Promise.all([getJSON(`${API_BASE}/daily?days=3660`),getJSON(`${API_BASE}/stats`),getJSON(`${API_BASE}/events`)]);dailyRows=Array.isArray(daily.days)?daily.days:[];dailyMap=new Map(dailyRows.map(row=>[row.day,row]));renderCharts();renderStats(stats);renderEvents(Array.isArray(events.events)?events.events:[]);renderHeatmap();const latest=dailyRows.length?dailyRows[dailyRows.length-1].day:null;if(latest){const input=$("archiveDateSearch");if(input){input.min=dailyRows[0].day;input.max=todayKey();input.value=latest;}calendarCursor=cursorFromDay(latest);selectedArchiveDay=latest;renderCalendar();loadDay(latest);}}catch(error){console.error("History:",error);set("archiveSearchStatus","Archive temporarily unavailable.");}}
...
// Refresh today while the page is open; historical selections stay selected.
setInterval(()=>{if(!document.hidden && selectedArchiveDay===todayKey())loadDay(selectedArchiveDay);},300000);
document.addEventListener("visibilitychange",()=>{if(!document.hidden && selectedArchiveDay===todayKey())loadDay(selectedArchiveDay);});
