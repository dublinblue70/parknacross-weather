...
function availableDays(){return dailyRows.map(row=>row.day).filter(day=>/^\d{4}-\d{2}-\d{2}$/.test(String(day))).sort();}
function updateArchiveDayNavigation(){const days=availableDays(),index=days.indexOf(selectedArchiveDay),previous=$("previousArchiveDay"),next=$("nextArchiveDay");if(previous){previous.disabled=index<=0;previous.dataset.day=index>0?days[index-1]:"";}if(next){next.disabled=index<0||index>=days.length-1;next.dataset.day=index>=0&&index<days.length-1?days[index+1]:"";}}
