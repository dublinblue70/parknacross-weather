...
function renderHeatmap(){
  ...
  for(let i=0;i<371;i++){
    ...
    if(row&&usable(row.sample_count)){
      ...
      const level=pct>=98?4:pct>=90?3:pct>=50?2:1;button.classList.add("has-data",`level-${level}`);button.title=`${longDay(day)} · ${actual}/${expected} expected readings saved · ${pct.toFixed(1)}% complete`;button.addEventListener("click",()=>openArchiveDay(day));
  ...
  set("heatmapSummary",populated?`${populated} archived day${populated===1?"":"s"} in view · ${complete} at ≥98% coverage${pct!==null?` · ${pct.toFixed(1)}% of expected readings saved across populated days`:""}.` : "The completeness map will fill as the archive grows.");
}
