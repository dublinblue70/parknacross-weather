  ...
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (url.pathname === "/met/forecast") {
        return json(await getMetForecast(), 200, {
          "Cache-Control": "public, max-age=300"
        });
      }

      if (url.pathname === "/met/warnings") {
        return json(await getMetWarnings(), 200, {
...
async function buildStats(env) {
  ...
  const [
    ...
    env.DB.prepare(`SELECT MIN(epoch) AS epoch FROM ${TABLE}`).first(),
  ...
  return {
    ...
    first_epoch: nullableNumber(first?.epoch),
...
function formatRow(row) {
  return {
    ...
    battery_v: nullableNumber(row.battery_v)
  };
}
